...
- CI must fail fast when release signing inputs are missing/invalid (keystore + alias + passwords).
- CI must publish signed APK outputs only; unsigned release artifacts must not be uploaded.
